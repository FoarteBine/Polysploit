...
static Gdiplus::Color mkcol(float r, float g, float b, float a) {
...
void Renderer::render_frame() {
    ...
    { std::lock_guard<std::mutex> lock(mtx_); cmds.swap(cmds_); }
    ...
    if (!cmds.empty()) {
        ...
        for (auto& c : cmds) {
            Gdiplus::Color color = mkcol(c.r, c.g, c.b, c.a);
            switch (c.type) {
            case DrawLine: {
            ...
            case DrawRect: {
                ...
                if (c.filled) { Gdiplus::SolidBrush brush(color); g.FillRectangle(&brush, rect); }
                else { Gdiplus::Pen pen(color, c.thickness > 0 ? c.thickness : 1.0f); g.DrawRectangle(&pen, rect.X, rect.Y, rect.Width, rect.Height); }
                break;
            }
            case DrawCircle: {
                float d = c.radius * 2;
                Gdiplus::RectF rect(c.x1 - c.radius, c.y1 - c.radius, d, d);
                if (c.filled) { Gdiplus::SolidBrush brush(color); g.FillEllipse(&brush, rect); }
                else { Gdiplus::Pen pen(color, c.thickness > 0 ? c.thickness : 1.0f); g.DrawEllipse(&pen, rect); }
                break;
            }
            case DrawTextCmd: {
    ...
    SIZE sz = { w, h };
    POINT pos = { pt.x, pt.y }; POINT src = { 0, 0 };
    UpdateLayeredWindow(overlay_hwnd_, hdcScreen, &pos, &sz, hdcMem, &src, 0, &blend, ULW_ALPHA);

    SelectObject(hdcMem, oldBmp);
    DeleteObject(hBitmap); DeleteDC(hdcMem); ReleaseDC(NULL, hdcScreen);
}
...
void Renderer::init() {
    ...
    Gdiplus::GdiplusStartup(&gdi_token_, &gsi, NULL);
    thread_ = CreateThread(NULL, 0, thread_proc, this, 0, NULL);
}

void Renderer::shutdown() {
    if (overlay_hwnd_) PostMessage(overlay_hwnd_, WM_CLOSE, 0, 0);
    if (thread_) {
        WaitForSingleObject(thread_, 2000);
        CloseHandle(thread_);
        thread_ = NULL;
    }
    if (gdi_token_) { Gdiplus::GdiplusShutdown(gdi_token_); gdi_token_ = 0; }
}
